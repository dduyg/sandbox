...
import { MarchingCubes } from "https://esm.sh/three@0.136.0/examples/jsm/objects/MarchingCubes.js";
...
function getBody({ debug = false, RAPIER, world }) {
  ...
  function update() {
    ...
    return pos; // Return the Vector3 object
  }
...
function animate() {
  ...
  metaballs.reset();
  bodies.forEach((b) => {
    const pos = b.update(); // pos is now a Vector3
    metaballs.addBall(pos.x, pos.y, pos.z, 0.5, 10, b.color.getHex());
  });
